...
public class NetTest extends Test {
  ...
  @Override public void init() {
    ...
    float x = 10;
    x = addButton("Google", () -> loadURL("http://www.google.com/"), x, 10);

    x = addButton("Enter URL", () -> {
      getText("Enter URL:").onSuccess(ifText(url -> loadURL(url)));
    }, x, 10);

    x = addButton("Post Test", () -> {
      getText("Enter POST body:").onSuccess(ifText(data -> {
        ...
        b.execute().onFailure(displayError).onSuccess(rsp -> {
    ...
    x = addButton("Fetch Posted Body", () -> {
      ...
      else game.net.req(lastPostURL).execute().onFailure(displayError).
    ...
    x = addButton("WS Send", () -> {
      ...
      else getText("Enter message:").onSuccess(ifText(msg -> {
  ...
  protected void loadURL (String url) {
    ...
    try {
      game.net.req(url).execute().onSuccess(this::displayResult).onFailure(displayError);
    } catch (Exception e) {
  ...
  protected final Slot<Throwable> displayError = error -> displayText(error.toString());

  private static Slot<String> ifText (Slot<String> op) {
    return text -> {
      if (text != null && text.length() > 0) op.onEmit(text);
    };
  }
}
