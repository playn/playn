...
public class CAFLoader {
  ...
  public static class CAFDesc {
     ...
     * frames per packet to determine the amount of time represented by a packet. This value must be
     * nonzero.
     */
    ...
    /**
     * The number of bytes in a packet of data. For formats with a variable packet size, this field
     * is set to 0. In that case, the file must include a Packet Table chunk “Packet Table Chunk.”
     * Packets are always aligned to a byte boundary.
     */
    ...
    /**
     * The number of bits of sample data for each channel in a frame of data. This field must be set
     * to 0 if the data format (for instance any compressed format) does not contain separate
    ...
    public int getALFormat() {
      switch (channelsPerFrame) {
        case 1:
          return (bitsPerChannel == 8) ? AL_FORMAT_MONO8 : AL_FORMAT_MONO16;
        case 2:
          return (bitsPerChannel == 8) ? AL_FORMAT_STEREO8 : AL_FORMAT_STEREO16;
        default:
    ...
    public String toString() {
      return String.format(
          ...
          sampleRate, formatID, formatFlags, bytesPerPacket, framesPerPacket, channelsPerFrame,
          bitsPerChannel);
    }
  }
