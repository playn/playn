...
class OldAndroidTextLayout extends AbstractTextLayout implements AndroidCanvas.Drawable {
  ...
  OldAndroidTextLayout(String text, TextFormat format) {
    super(text, format);
  ...
  void breakLine(String text) {
    ...
    while (start < end) {
      ...
      if (lineEnd < end && font.ligatureHacks.length > 0) {
        int adjust = AndroidTextLayout.accountForLigatures(text, start, count, font.ligatureHacks);
        count += adjust;
        lineEnd += adjust;
      }
