...
public class HtmlAnalytics implements Analytics {
  ...
  /**
   * Random value, determined once per game / session, so that multiple events
   * with the same sample rate will be consistently logged -or- not logged in
   * any given session. We don't use Google Analytics' <a href=
   * "http://code.google.com/apis/analytics/docs/gaJS/gaJSApiBasicConfiguration.html#_gat.GA_Tracker_._setSampleRate"
   * >{@literal _setSampleRate}</a> here because that would affect all events in
   * the session.
   */
  private float random;

  public HtmlAnalytics() {
    random = ForPlay.random();
    String val = Window.Location.getParameter("analytics");
    if (val != null) {
      try {
  ...
  public native void logEventImpl(String category, String action) /*-{
		$wnd._gaq.push([ '_trackEvent', category, event ]);
  }-*/;
