...
public class IndexedTrisShader extends GLShader {
  ...
  private static final int VERTEX_SIZE = 11; // floats per vertex
  ...
  protected class ITCore extends Core {
    ...
    public ITCore(String vertShader, String fragShader) {
      ...
      aColor = prog.getAttrib("a_Color", 4, GL20.GL_UNSIGNED_BYTE);
    ...
    public void addQuad(float m00, float m01, float m10, float m11, float tx, float ty,
      ...
      vertices.add(m00, m01, m10, m11, tx, ty).add(x1, y1).add(sx1, sy1).add(tint);
      vertices.add(m00, m01, m10, m11, tx, ty).add(x2, y2).add(sx2, sy2).add(tint);
      vertices.add(m00, m01, m10, m11, tx, ty).add(x3, y3).add(sx3, sy3).add(tint);
      vertices.add(m00, m01, m10, m11, tx, ty).add(x4, y4).add(sx4, sy4).add(tint);
    ...
    public void addTriangles(float m00, float m01, float m10, float m11, float tx, float ty,
      ...
      for (int ii = 0, ll = xys.length; ii < ll; ii += 2) {
        ...
        vertices.add(m00, m01, m10, m11, tx, ty).add(x, y).add(x/tw, y/th).add(tint);
      }
